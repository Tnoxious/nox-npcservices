...
local function GetUsername(id)
    return QBCore.Functions.GetPlayer(id).PlayerData.charinfo.firstname ..
        " " .. QBCore.Functions.GetPlayer(id).PlayerData.charinfo.lastname
end
    ...
    if Player then
        if
            Player.PlayerData.job.name == "police" or Player.PlayerData.job.name == "ambulance" or
                ...
                Player.PlayerData.job.name == "taxi" and Player.PlayerData.job.onduty
         then
            isAllowed = true
        end
    end
...
QBCore.Functions.CreateCallback(
    "mh-npcservices:server:emsOnline",
...
QBCore.Functions.CreateCallback(
    ...
    function(source, cb, company, price)
        local src = source
...
RegisterServerEvent(
    "mh-npcservices:server:sendService",
    function(callData)
        local src = source
        local num = nil
        if not id then
        ...
        if src == callData.targetId then
            num = src
        else
            num = callData.targetId
        end
            ...
            if count >= Config.MinOnlineEMS then
                ...
                if CanIPayTheBill(num, callData.job, callData.price) then
                    ...
                    TriggerClientEvent(
                        "QBCore:Notify",
                        num,
                        Lang:t("notify.cant_pay", {price = callData.price}),
                        "error",
                        10000
                    )
                end
...
RegisterServerEvent(
    "mh-npcservices:server:getin",
    function(job, vehicle)
        local src = source
...
RegisterServerEvent(
    "mh-npcservices:server:leavevehicle",
    function(vehicle)
...
RegisterServerEvent(
    "mh-npcservices:server:pay",
    function(company, price)
        local src = source
        local Player = QBCore.Functions.GetPlayer(src)
